...
/* CONFIG */
const node = "/Users/thanhhoann/.local/share/nvm/v20.8.0/bin/node"; // node executable path
...
const readData = () =>
  new Promise((resolve) =>
    run(`cat ${dataFile}`).then((data) => resolve(JSON.parse(data || "null"))),
  );
...
const closeEvent = (e) =>
  run("osascript -l JavaScript ./Memos.widget/lib/confirm.scpt").then(
    (answer) => {
      if (answer.trim() === "OK") {
...
const MemoWrapper = ({ state, dispatch }) => {
  ...
  useEffect(() => {
    if (text) textareaRef.current.value = text;
