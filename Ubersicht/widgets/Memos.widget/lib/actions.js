...
const _SCREEN_SIZE_ = 8;
const [screenWidth, screenHeight] = [_SCREEN_SIZE_]; // CONFIG SCREEN SIZE HERE
...
const reloadApp = () =>
  execFile("osascript", [
    ...
    "JavaScript",
    "-e",
    'delay(0.5);Application("tracesOf.Uebersicht").widgets.byId("Memos-widget-index-jsx").reload()',
  ]);
  ...
  while (overlap) {
    ...
    overlap = existingMemos.some((existingMemo) => {
      ...
      const overlapsHorizontally =
        newRight >= existingLeft && left <= existingRight;
      const overlapsVertically =
        newBottom >= existingTop && top <= existingBottom;
...
const editMemo = (id, text = "", append = !1) => {
  ...
  data[id].text = append
    ? data[id].text +
      (data[id].text && !data[id].text.endsWith("\n") ? "\n" : "") +
      text
    : text;
  writeData(data);
  reloadApp();
...
((mode) => {
  if (mode === "init") {
    ...
    console.log(
      "Usage: node actions.js [init|new|delete|edit|append|list] [memoId] [text]",
    );
  }
})(process.argv[2]);
